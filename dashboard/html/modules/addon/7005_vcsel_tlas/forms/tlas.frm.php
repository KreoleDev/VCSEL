...
/*
*   2019.03.01  CP  Completed for and added tab for Test Structure
*/
...
if($common['security']->check_rights(1)){
    ...
    function insert(){
        ...
        if(!empty($errors) || !$common['security']->verify_frm()){
            ...
            $affected=$common['db']->pec('INSERT INTO 2019_prod_tlas SET ext_product_id=5, tla_number=?, active=?',array($_POST['tla_number'],$_POST['active']),'si');
            if(!$affected){ $success=false; }
            
            $tla_id=$common['db']->last_insert_id();
            
            //Create first version
            $affected=$common['db']->pec('INSERT INTO 2019_prod_tla_info_product_5 SET ext_tla_id=?, rev_timestamp=NOW()',
                                            array($tla_id),
                                            'i');
            if(!$affected){ $success=false; }
    ...
    switch($mode){
        ...
        case 'edit':
            ...
            /*$sub_info=$common['db']->pec('SELECT ext_top_cis_id, ext_bottom_cis_id, has_micr, has_stamp, has_watermark, sensor_type, has_encryption, ext_firmware_id, usb_descriptor, ext_tolerance_id, ext_reject_id FROM 2019_prod_tla_info_product_3 WHERE ext_tla_id=? ORDER BY rev_timestamp DESC LIMIT 1',
                array($_REQUEST['tla_id']),'i',
                array('ext_top_cis_id', 'ext_bottom_cis_id', 'has_micr', 'has_stamp', 'has_watermark', 'sensor_type', 'has_encryption', 'ext_firmware_id', 'usb_descriptor', 'ext_tolerance_id', 'ext_reject_id'));
            if(isset($sub_info[0])) {
                $tla_info[0]['ext_top_cis_id'] = $sub_info[0]['ext_top_cis_id'];
                $tla_info[0]['ext_bottom_cis_id'] = $sub_info[0]['ext_bottom_cis_id'];
                $tla_info[0]['has_micr'] = $sub_info[0]['has_micr'];
                $tla_info[0]['has_stamp'] = $sub_info[0]['has_stamp'];
                $tla_info[0]['has_watermark'] = $sub_info[0]['has_watermark'];
                $tla_info[0]['sensor_type'] = $sub_info[0]['sensor_type'];
                $tla_info[0]['has_encryption'] = $sub_info[0]['has_encryption'];
                $tla_info[0]['ext_firmware_id'] = $sub_info[0]['ext_firmware_id'];
                $tla_info[0]['usb_descriptor'] = $sub_info[0]['usb_descriptor'];
                $tla_info[0]['ext_tolerance_id'] = $sub_info[0]['ext_tolerance_id'];
                $tla_info[0]['ext_reject_id'] = $sub_info[0]['ext_reject_id'];
            }*/
