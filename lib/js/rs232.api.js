...
try {
    ...
    if (serialPort.SerialPort) {
        ...
        serialPort = function (comPort, options, callback) {
            return new SerialPortClass(Object.assign({ path: comPort }, options), callback);
        };

        serialPort.list = function(callback) {
            SerialPortClass.list().then((ports) => {
                callback(null, ports.map((port) => {
                    if(port.comName === undefined) {
                        port.comName = port.path;
                    }
                    if(port.vendorId !== undefined) {
                        port.vendorId = String(port.vendorId).toLowerCase();
                    }
                    if(port.productId !== undefined) {
                        port.productId = String(port.productId).toLowerCase();
                    }
                    return port;
                }));
            }).catch((err) => {
    ...
    serialPort = function () {
        throw new Error('Serial port native module is not installed in this build');
    };
    serialPort.list = function(callback) {
        callback(null, []);
    };
        ...
        if(byteIndex < byteArray.length) {
            ...
            serialPortInstance.get((error,modemBits)=>{
                if(error) {
                    ...
                    if(lastRtscts && modemBits.cts == false) {
                        console.log('Serial Busy: cts = ' + modemBits.cts + ' dsr = ' + modemBits.dsr);
                        if(attemptTimeInTenthsSeconds > 0) {
                            setTimeout(() => {
                                sendSerialByteIndexIgnoreDSR(byteArray, byteIndex, attemptTimeInTenthsSeconds - 1).then(() => {
        ...
        value: function connect(comPort, baudRate, dataBits, stopBits, parity, rtscts, xon, xoff) {
            ...
            return new Promise(function (resolve, reject) {
                if(!serialPortNativeAvailable) {
                    reject('Serial port native module is not installed in this build');
                    return;
                }
                lastComPort = comPort;
