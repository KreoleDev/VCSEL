		            ...
		            echo $frm->begin_fieldset('General Information');
                        echo $frm->begin_dl();
                            if($hidden['mode']=='insert') {
                                ...
                                $results=$common['db']->pec('SELECT test_id, title, description FROM 2019_prod_tests WHERE test_id IN (SELECT ext_test_id FROM 2019_prod_test_product_assoc WHERE ext_product_id=5) AND test_id NOT IN(SELECT ext_test_id FROM 2019_prod_tla_test_assoc WHERE ext_tla_id=?) ORDER BY title',array($hidden['ext_tla_id']),'i',array('test_id', 'title', 'description'));
                                $tests=array();
		        ...
		        echo $frm->begin_frm();
		            echo $frm->begin_fieldset('General Information');
                        echo $frm->begin_dl();
                            ...
                            $results = $common['db']->pec('SELECT tla_id, tla_number FROM 2019_prod_tlas WHERE ext_product_id=5 AND tla_id<>? ORDER BY tla_number',array($hidden['ext_tla_id']),'i',array('tla_id', 'tla_number'));
