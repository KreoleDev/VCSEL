...
if($common['security']->check_rights(1)){
    ...
    function insert(){
        ...
        if(!empty($errors) || !$common['security']->verify_frm()){
            ...
            $affected=$common['db']->pec('INSERT INTO 2019_prod_tlas SET ext_product_id=7, tla_number=?, active=?',array($_POST['tla_number'],$_POST['active']),'si');
            if(!$affected){ $success=false; }
            
            $tla_id=$common['db']->last_insert_id();
            
            //Create first version
            $affected=$common['db']->pec('INSERT INTO 2019_prod_tla_info_product_7 SET ext_tla_id=?, rev_timestamp=NOW()',
                                            array($tla_id),
                                            'i');
            if(!$affected){ $success=false; }
