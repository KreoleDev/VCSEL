...
function validate(){
    ...
    if(empty($_POST['current_password'])){
        ...
        $valid_password=$common['db']->pec('SELECT user_id FROM core_users WHERE user_id=? AND password=SHA1(CONCAT(?,salt)) LIMIT 1',array($_SESSION['user_id'],$_POST['current_password']),'is',array('user_id'));
        if(!isset($valid_password[0]['user_id'])){
