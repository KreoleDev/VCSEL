...
class security{
    ...
    private function login_local($site_path, $username, $user_id, $password, $local_password_expire_days, $max_login_attempt_count, $failed_login_lockout_minutes){
        ...
        if($failed_attempts[0]['count']>=$max_login_attempt_count){
            ...
            $valid_user=$site_db->pec('SELECT user_id, active, account_expires, expiry_date FROM core_users WHERE username=? AND password=SHA1(CONCAT(?,salt)) LIMIT 1',array($username,$password),'ss',array('user_id','active','account_expires', 'expiry_date'));
            if(!empty($valid_user)){
                //valid password
